...
def palindrome(d):
    result = True if str(d) == str(d)[::-1] else False  #Тернарный оператор это очень хорошо
    return result
...
def inside(s):
    s = s.split(" ")
    first_half = [" ".join(s[1: int(len(s)/2)]), s[0]]
    second_half = [" " + s[-1], " ".join(s[int(len(s)/2):-1])]
